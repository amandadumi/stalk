...
class LineSearchBase(LineSearchGrid):
    ...
    def search_with_error(self, **ls_overrides):
        settings = self.settings.copy(**ls_overrides)
        res = settings.fit_func.find_noisy_minimum(
            self, sgn=settings.sgn, fraction=settings.fraction, N=settings.N
        )
        return res

    # end def

    def search(self, **ls_overrides):
        settings = self.settings.copy(**ls_overrides)
        res = settings.fit_func.find_minimum(self, sgn=settings.sgn)
        return res
    ...
    def plot(self, ax=None, color="tab:blue", target=None, **kwargs):
        ...
        if self.fit_res is not None:
            ...
            ax.plot(xgrid, ygrid, linestyle="--", color=color, label="Fitted curve")
        # end if
        plt.tight_layout()
