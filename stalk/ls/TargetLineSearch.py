...
from numpy import array, isscalar, linspace, nan, ndarray
...
class TargetLineSearch(TargetLineSearchBase, LineSearch):
    ...
    def sigma_opt(self, sigma_opt):
        if isscalar(sigma_opt) and sigma_opt > 0.0 or sigma_opt is None:
            ...
            raise ValueError(
                "sigma_opt must be > 0.0 but " + str(sigma_opt) + " was given."
            )
        # end if
    ...
    def epsilon(self, epsilon):
        if isscalar(epsilon) and epsilon > 0.0 or epsilon is None:
            ...
            raise ValueError(
                "Epsilon must be > 0.0 but " + str(epsilon) + " was given."
            )
        # end if
    ...
    def grid_opt(self):
        ...
        grid = LineSearchGrid(offsets=offsets, errors=errors)
        return grid
        ...
        **ls_args,  # sgn, fraction, fit_func, N, Gs, bias_order, bias_mix
        ...
        for W in Ws:
            ...
            bias = self.compute_bias(
                grid=LineSearchGrid(offsets=offsets, values=values), **ls_args
            )
            biases.append(bias)
        # end for
    ...
    def figure_out_adjusted_offsets(self, **grid_args):  # M, R, W, offsets
        adjusted_offsets = (
            self.figure_out_offsets(**grid_args) + self.target_settings.target.x0
        )
    ...
    def generate_error_surface(
        ...
        self._error_surface = ErrorSurface(
            X_res=W_resolution, Y_res=S_resolution, verbosity=verbosity
        )
    ...
    def insert_W_data(self, W):
        ...
        # end if
        E_col = [
            self._compute_target_error(W, sigma) for sigma in self.error_surface.Ys
        ]
    ...
    def plot_error_surface(self, ax=None):
        if not self.optimized:
        ...
        if ax is None:
            ...
            ax.set_title(
                f"Error surface: {repr(self)}, epsilon={FF.format(self.epsilon)}"
            )
        # end if
