...
class PolynomialFit(FittingFunction):
    ...
    def n(self, n):
        ...
        if n < 2:
            raise ValueError(
                f"Polynomial degree n must be higher than 1, provided: {n}"
            )
        # end if
        self._n = n
    ...
    def _eval_function(self, offsets, values, derivs=None) -> PolynomialResult:
        if len(offsets) <= self.n:
            raise ValueError(
                f"Fitting of {self.n} degree polynomial to {len(offsets)} points is under-determined. Aborting."
            )
        # end if
