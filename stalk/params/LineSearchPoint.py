...
from numpy import abs, isnan, isscalar
...
class LineSearchPoint:
    ...
    def derivative_err(self, derivative_err):
        if isscalar(derivative_err):
            self._derivative_err = derivative_err

    def has_deritvative(self) -> bool:
        if self_derivative is not None:
            return True
        else:
            return False
    ...
    def plot(self, ax: Axes, color="tab:blue", marker="o", linestyle="none", **kwargs):
        if self.error > 0:
            ax.errorbar(
                self.offset,
                self.value,
    ...
    def __eq__(self, other):
        return (
            isinstance(other, LineSearchPoint)
            and abs(self.offset - other.offset) < self.tol
        )
