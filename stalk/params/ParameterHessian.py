...
class ParameterHessian:
    ...
    def hessian(self, hessian):
        ...
        if hessian is None:
            ...
            if (
                len(hessian.shape) != 2
                or hessian.shape[0] != d
                or hessian.shape[1] != d
            ):
                raise AssertionError(
                    f"The Hessian must be {d}x{d} array, provided: {hessian.shape}"
                )
            # end if
    ...
    def compute_fdiff(
        ...
        **kwargs,
        ...
        if isscalar(dp):
            ...
            raise ValueError(
                f"Error: Provided {len(dp)} dps for {P} directions! Aborting."
            )
        # end if
    ...
    def _warn_energy(self, structure_list: list[ParameterSet]):
        ...
        for structure in structure_list[0:]:
            if structure.value < eqm_value:
                warnings.warn(
                    f"Offset energy lower than eqm: E({structure.label})={structure.value} < E(eqm)={eqm_value}!"
                )
            # end if
        # end for
    ...
    def __str__(self):
        ...
        if self.hessian is not None:
            ...
            for Lambda, direction in zip(self.lambdas, self.directions):
                string += ("\n    {:<8f}   " + len(direction) * "{:<+1.6f} ").format(
                    Lambda, *tuple(direction)
                )
            # end for
        else:
            string += "\n  hessian: not set"
